...
class CashfreeService {
  ...
  async createPaymentSession(order, customer) {
    if (!this.isConfigured()) {
      throw new Error(
        'Cashfree is not configured. Please set CASHFREE_APP_ID and CASHFREE_SECRET_KEY'
      );
    }
   ...
   * @param {Object|Buffer} payload - Webhook payload (raw buffer preferred)
  ...
  verifyWebhookSignature(payload, signature, timestamp) {
    ...
    try {
      // Cashfree signature is: timestamp + rawBody
      // payload here should ideally be the raw buffer
      const data = Buffer.isBuffer(payload) ? payload : Buffer.from(JSON.stringify(payload));
      const signatureData = Buffer.concat([Buffer.from(timestamp), data]);
  ...
  async getPaymentStatus(orderId) {
    ...
    try {
      ...
      logger.error(
        `Cashfree status check error: ${error.response?.data?.message || error.message}`
      );
  ...
  async getPaymentDetails(orderId) {
    ...
    try {
      ...
      logger.error(
        `Cashfree payment details error: ${error.response?.data?.message || error.message}`
      );
