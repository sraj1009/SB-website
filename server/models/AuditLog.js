...
auditLogSchema.statics.getActorLogs = function (actorId, limit = 100) {
  return this.find({ actor: actorId }).sort({ createdAt: -1 }).limit(limit).lean();
};
...
auditLogSchema.statics.getSecurityLogs = function (limit = 100) {
  return this.find({
    action: {
      $in: ['SUSPICIOUS_ACTIVITY', 'RATE_LIMIT_EXCEEDED', 'INVALID_TOKEN_ATTEMPT', 'USER_BANNED'],
    },
  })
    .sort({ createdAt: -1 })
    .limit(limit)
