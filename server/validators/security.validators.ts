...
export const baseSchemas = {
  ...
  pagination: z.object({
    page: z.coerce.number().min(1, 'Page must be at least 1').max(1000, 'Page cannot exceed 1000').default(1),
    limit: z.coerce.number().min(1, 'Limit must be at least 1').max(100, 'Limit cannot exceed 100').default(20),
  }),
  
  // Sort validation
  sort: z.object({
    sortBy: z.enum(['createdAt', 'updatedAt', 'name', 'price', 'rating'], 'Invalid sort field').default('createdAt'),
...
export const productSchemas = {
  ...
  productQuery: z.object({
    ...
    category: z.enum(['Books', 'Poem Book', 'Story Book', 'Stationeries', 'Foods', 'Honey']).optional(),
    status: z.enum(['active', 'inactive', 'out_of_stock']).optional(),
...
export const paymentSchemas = {
  ...
  initiatePayment: z.object({
    ...
    amount: z.number().min(1, 'Amount must be at least 1').max(999999, 'Amount cannot exceed 999,999'),
