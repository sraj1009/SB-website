...
export const createPaymentSession = async (req, res, next) => {
    try {
        ...
        for (const item of items) {
            ...
            if (!prod) return res.status(404).json({ success: false, error: { message: `Product ${item.product} not found` } });
            if (prod.stockQuantity < item.quantity) {
                return res.status(400).json({ success: false, error: { message: `Insufficient stock for ${prod.title}. Only ${prod.stockQuantity} left.` } });
            }
        }

        // Decrement stock to soft-lock it
        for (const item of items) {
            await Product.findByIdAndUpdate(item.product, { $inc: { stockQuantity: -item.quantity, countInStock: -item.quantity, stock: -item.quantity } });
        }
...
export const handleWebhook = async (req, res, next) => {
    try {
        ...
        if (cashfreeService.isConfigured()) {
            const isValid = cashfreeService.verifyWebhookSignature(payload, signature, timestamp);
        ...
        logger.info(`Received Cashfree webhook: ${eventType} for session/order ${data?.order?.order_id}`);
                ...
                if (session && !session.orderCreated) {
                    ...
                    try {
                        ...
                        await dbSession.withTransaction(async () => {
                            ...
                            logger.info(`Order ${order.orderId} created via Cashfree webhook for session ${sessionId}`);
                        });
        ...
        for (const session of expiredSessions) {
            ...
            for (const item of session.items) {
                await Product.findByIdAndUpdate(item.product, {
                    $inc: { stockQuantity: item.quantity, countInStock: item.quantity, stock: item.quantity }
                });
            }
