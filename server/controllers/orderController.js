...
export const createOrder = async (req, res, next) => {
    try {
        const { items, shippingAddress, paymentMethod = 'upi_manual', notes } = req.body;
        ...
        const order = await runInTransaction(async (session) => {
            ...
            for (const item of items) {
                ...
                if (!product) {
                    throw { statusCode: 404, code: 'PRODUCT_NOT_FOUND', message: `Product ${item.productId} not found` };
                }

                if (product.isDeleted || product.status === 'disabled') {
                    throw { statusCode: 400, code: 'PRODUCT_UNAVAILABLE', message: `${product.title} is no longer available` };
                }

                if (product.stock < item.quantity) {
                    throw { statusCode: 400, code: 'INSUFFICIENT_STOCK', message: `Insufficient stock for ${product.title}. Available: ${product.stock}` };
                }
