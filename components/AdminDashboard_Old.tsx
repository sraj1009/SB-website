...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ onClose }) => {
  ...
  const handleUpdateUserStatus = async (
    ...
    if (
      !confirm(
        `Are you sure you want to ${status === 'banned' ? 'ban' : status === 'suspended' ? 'suspend' : 'activate'} this user?`
      )
    )
      return;
    try {
          ...
          <div className="flex items-center gap-3">
            ...
            {!sidebarCollapsed && (
              <div className="overflow-hidden">
                <h1 className="text-white font-black text-sm tracking-tight truncate">
                  SINGGLEBEE
                </h1>
                <p className="text-amber-500/60 text-[9px] font-bold uppercase tracking-[0.2em]">
                  Admin Console
                </p>
              </div>
            )}
      ...
      <main className="flex-1 flex flex-col overflow-hidden bg-gray-50">
        <header className="h-16 shrink-0 bg-white border-b border-gray-100 flex items-center justify-between px-8">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={fetchData}
              className="w-9 h-9 rounded-xl bg-gray-100 flex items-center justify-center text-gray-500 hover:bg-amber-50 hover:text-amber-600 transition-all text-sm"
              title="Refresh"
            >
              🔄
            </button>
        ...
        <div className="flex-1 overflow-y-auto p-6 lg:p-8">
          {isLoading ? (
            ...
            <>
              ...
              {activeTab === 'customers' && (
                <CustomersTab customers={customers} onUpdateStatus={handleUpdateUserStatus} />
              )}
            </>
          )}
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {stats.ordersByStatus &&
          Object.entries(stats.ordersByStatus).map(([status, count]: any) => (
            <div
              key={status}
              className="bg-gray-50 rounded-xl p-4 text-center hover:bg-amber-50 transition-colors group"
            >
              <span
                className={`inline-block px-2.5 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest mb-2 ${getStatusColor(status)}`}
              >
                {status}
              </span>
        ...
        {stats.recentOrders?.length > 0 ? (
          <div className="space-y-3">
            {stats.recentOrders.map((order: any) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg bg-amber-100 flex items-center justify-center text-xs">
                    📦
                  </div>
                  <div>
                ...
                <div className="text-right">
                  ...
                  <span
                    className={`text-[9px] font-bold uppercase ${getStatusColor(order.status)}`}
                  >
                    {order.status}
                  </span>
                </div>
        ...
        {stats.lowStockProducts?.length > 0 ? (
          <div className="space-y-3">
            {stats.lowStockProducts.map((product: any) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg bg-rose-100 flex items-center justify-center text-xs">
                    ⚠️
                  </div>
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div
          className="bg-emerald-50 rounded-xl border border-emerald-100 p-4 text-center cursor-pointer hover:bg-emerald-100 transition-colors"
          onClick={() => onPaymentFilterChange(paymentFilter === 'paid' ? 'all' : 'paid')}
        >
        ...
        <div
          className="bg-amber-50 rounded-xl border border-amber-100 p-4 text-center cursor-pointer hover:bg-amber-100 transition-colors"
          onClick={() => onPaymentFilterChange(paymentFilter === 'unpaid' ? 'all' : 'unpaid')}
        >
              ...
              <tbody className="divide-y divide-gray-50">
                {orders.map((order) => (
                  <tr key={order._id} className="hover:bg-amber-50/30 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`inline-block px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-wider ${getStatusColor(order.status)}`}
                      >
                        {order.status}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex flex-col gap-1">
                        ...
                        {order.payment?.method === 'upi_manual' &&
                          order.payment?.status !== 'success' && (
                            <button
                              ...
                              className="text-[9px] font-black text-amber-600 bg-amber-50 border border-amber-200 px-2 py-0.5 rounded hover:bg-amber-100 transition-colors w-fit"
                            >
                              ✓ Verify UPI
                            </button>
                          )}
                        {order.payment?.status !== 'success' && (
                          <button
                            onClick={() => onVerifyPayment(order._id)}
                            className="text-[9px] font-black text-emerald-600 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded hover:bg-emerald-100 transition-colors w-fit"
                          >
                            💳 Mark Paid
                          </button>
                        )}
            ...
            >
              <div className="flex gap-4">
                <div className="w-16 h-16 rounded-xl bg-gray-50 overflow-hidden shrink-0 border border-gray-100">
                  <img
                    src={product.images?.[0] || '/assets/bee-character.png'}
                    alt={product.title}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-gray-400 font-bold mt-0.5">
                    {product.category} • {product.language || 'N/A'}
                  </p>
                  <div className="flex items-center justify-between mt-2">
                    <span className="text-sm font-black text-amber-600">₹{product.price}</span>
                    <span
                      className={`text-[10px] font-black px-2 py-0.5 rounded-full ${stock <= 5 ? 'bg-rose-100 text-rose-600' : 'bg-emerald-100 text-emerald-600'}`}
                    >
                      {stock} in stock
                    </span>
                  </div>
              ...
              <div className="flex gap-2 mt-4 pt-4 border-t border-gray-50">
                <span className="flex-1 text-center bg-gray-50 text-gray-500 font-bold py-2 rounded-lg text-[10px]">
                  ★ {product.rating || '—'}
                </span>
                <span className="flex-1 text-center bg-gray-50 text-gray-500 font-bold py-2 rounded-lg text-[10px]">
                ...
                <span
                  className={`flex-1 text-center font-bold py-2 rounded-lg text-[10px] uppercase ${product.status === 'active' ? 'bg-emerald-50 text-emerald-600' : 'bg-gray-100 text-gray-400'}`}
                >
...
const CustomersTab = ({
  ...
  customers: any[];
  onUpdateStatus: (id: string, status: 'active' | 'banned' | 'suspended') => void;
}) => (
  <div className="space-y-5 animate-fade-in">
    <div className="flex items-center justify-between">
      ...
      <div className="flex gap-2 text-[10px] font-bold text-gray-400">
        ...
        <span className="bg-rose-50 text-rose-600 px-2 py-1 rounded">
              ...
              {customers.map((customer) => {
                ...
                return (
                  <tr key={customer._id} className="hover:bg-amber-50/30 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <code className="text-[9px] bg-gray-100 px-1.5 py-0.5 rounded font-mono text-gray-500">
                        {customer._id}
                      </code>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`px-2 py-0.5 rounded text-[9px] font-black uppercase ${customer.role === 'admin' ? 'bg-purple-100 text-purple-600' : 'bg-gray-100 text-gray-500'}`}
                      >
                    ...
                    <td className="px-6 py-4">
                      <div className="space-y-0.5">
                        <p className="text-[10px] text-gray-500">
                          Joined{' '}
                          <span className="font-bold text-gray-700">{daysSinceJoin}d ago</span>
                        </p>
                        {lastLogin && (
                          <p className="text-[10px] text-gray-400">
                        ...
                        {loginCount > 0 && (
                          <p className="text-[10px] text-gray-400">{loginCount} logins</p>
                        )}
                      </div>
                    </td>
                    <td className="px-6 py-4 text-right">
                      {customer.role !== 'admin' && (
                        <div className="flex gap-1 justify-end">
                          {customer.status !== 'active' && (
                            <button
                              onClick={() => onUpdateStatus(customer._id, 'active')}
                              className="text-[9px] font-black text-emerald-600 bg-emerald-50 border border-emerald-200 px-2 py-1 rounded hover:bg-emerald-100 transition-colors"
                            >
                              Activate
                            </button>
                          )}
                          {customer.status !== 'suspended' && (
                            <button
                              onClick={() => onUpdateStatus(customer._id, 'suspended')}
                              className="text-[9px] font-black text-amber-600 bg-amber-50 border border-amber-200 px-2 py-1 rounded hover:bg-amber-100 transition-colors"
                            >
                              Suspend
                            </button>
                          )}
                          {customer.status !== 'banned' && (
                            <button
                              onClick={() => onUpdateStatus(customer._id, 'banned')}
                              className="text-[9px] font-black text-rose-600 bg-rose-50 border border-rose-200 px-2 py-1 rounded hover:bg-rose-100 transition-colors"
                            >
                              Ban
                            </button>
                          )}
                      ...
                      {customer.role === 'admin' && (
                        <span className="text-[9px] text-gray-400 italic">Protected</span>
                      )}
                    </td>
                  </tr>
                );
...
const KPICard = ({
  ...
  <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm hover:shadow-md transition-all group relative overflow-hidden">
    <div
      className={`absolute top-0 right-0 w-20 h-20 rounded-full bg-gradient-to-br ${gradient} opacity-5 -mr-6 -mt-6 group-hover:opacity-10 transition-opacity`}
    ></div>
    <div className="flex items-start justify-between mb-3">
      <span className="text-2xl">{icon}</span>
      {trend && (
        <span
          className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${trendUp === true ? 'bg-emerald-50 text-emerald-600' : trendUp === false ? 'bg-rose-50 text-rose-600' : 'bg-gray-50 text-gray-500'}`}
        >
          {trend}
        </span>
      )}
...
const EmptyMini = ({ message }: { message: string }) => (
...
const LoadingState = ({ tab }: { tab: string }) => (
  <div className="space-y-5 animate-pulse">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="bg-white rounded-2xl border border-gray-100 p-5 h-32">
              <div className="w-8 h-8 bg-gray-100 rounded-lg mb-3"></div>
              <div className="h-3 bg-gray-100 rounded w-24 mb-2"></div>
              <div className="h-6 bg-gray-100 rounded w-16"></div>
            </div>
          ))}
        </div>
        <div className="bg-white rounded-2xl border border-gray-100 p-6 h-48"></div>
      </>
    )}
    ...
    {tab === 'products' && (
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {[...Array(6)].map((_, i) => (
          <div key={i} className="bg-white rounded-2xl border border-gray-100 p-5 h-40"></div>
        ))}
      </div>
    )}
  </div>
