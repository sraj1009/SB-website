...
const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-[60] font-sans">
      ...
      <nav
        className={`w-full transition-all duration-500 ${scrolled ? 'bg-white/80 backdrop-blur-xl shadow-sm py-1.5 md:py-2 border-b border-brand-primary/5' : 'bg-transparent py-4 md:py-6'}`}
      >
        <div className="container mx-auto px-6 lg:px-12 max-w-screen-2xl">
          <div className="flex items-center justify-between gap-6">
            {/* Logo Container */}
            <div
              className="flex items-center cursor-pointer group shrink-0 relative"
            ...
            <div className="flex items-center gap-2 sm:gap-3 md:gap-5 shrink-0">
              ...
              >
                ...
                />
                <svg
                  className="h-5 w-5 absolute left-4 top-1/2 -translate-y-1/2 text-brand-primary"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  strokeWidth={3}
              ...
              <div className="relative" ref={profileRef}>
                {user ? (
                  <button
                    ...
                    className="w-12 h-12 md:w-14 md:h-14 rounded-2xl border-4 border-white overflow-hidden bg-white shadow-honey active:scale-95 transition-all"
                  >
                    <img
                      src={user.avatar}
                      alt="Profile"
                  ...
                  <button
                    ...
                    className="flex h-10 sm:h-12 md:h-14 px-4 sm:px-7 items-center justify-center bg-white rounded-xl sm:rounded-2xl border-2 border-amber-200 text-[10px] sm:text-xs font-bold text-gray-800 hover:bg-amber-50 hover:border-amber-400 transition-all shadow-sm"
                  >
                    Sign In
                  </button>
                )}
                ...
                {isProfileOpen && user && (
                  <div className="absolute top-full right-0 mt-5 w-64 bg-white rounded-[2.5rem] shadow-honey-hover border border-brand-primary/5 overflow-hidden p-2 animate-slide-up origin-top-right">
                    ...
                    <button
                      ...
                      className="w-full text-left px-6 py-4 text-sm font-black text-brand-rose hover:bg-rose-50 transition-all flex items-center gap-3 rounded-b-[2rem]"
                    >
                      🚪 Log Out of Hive
                    </button>
                  </div>
