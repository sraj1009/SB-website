    ...
    try {
      ...
      for (const month of months) {
        ...
        Object.keys(monthlyReport.revenue.byCategory).forEach((key) => {
          quarterlyData.revenue.byCategory[key] =
            (quarterlyData.revenue.byCategory[key] || 0) + monthlyReport.revenue.byCategory[key];
        });

        Object.keys(monthlyReport.revenue.byChannel).forEach((key) => {
          quarterlyData.revenue.byChannel[key] =
            (quarterlyData.revenue.byChannel[key] || 0) + monthlyReport.revenue.byChannel[key];
        });
      }

      // Calculate quarterly metrics
      quarterlyData.profitability.grossMargin =
        (quarterlyData.profitability.grossProfit / quarterlyData.revenue.total) * 100;
      quarterlyData.profitability.operatingMargin =
        (quarterlyData.profitability.operatingProfit / quarterlyData.revenue.total) * 100;
      quarterlyData.profitability.netMargin =
        (quarterlyData.profitability.netProfit / quarterlyData.revenue.total) * 100;
      quarterlyData.profitability.ebitda =
        quarterlyData.profitability.operatingProfit + quarterlyData.expenses.capitalExpenditures;
      quarterlyData.profitability.ebitdaMargin =
        (quarterlyData.profitability.ebitda / quarterlyData.revenue.total) * 100;
    ...
    try {
      ...
      for (let quarter = 1; quarter <= 4; quarter++) {
        ...
        Object.keys(quarterlyReport.revenue.byCategory).forEach((key) => {
          annualData.revenue.byCategory[key] =
            (annualData.revenue.byCategory[key] || 0) + quarterlyReport.revenue.byCategory[key];
        });

        Object.keys(quarterlyReport.revenue.byChannel).forEach((key) => {
          annualData.revenue.byChannel[key] =
            (annualData.revenue.byChannel[key] || 0) + quarterlyReport.revenue.byChannel[key];
        });
      }

      // Calculate annual metrics
      annualData.profitability.grossMargin =
        (annualData.profitability.grossProfit / annualData.revenue.total) * 100;
      annualData.profitability.operatingMargin =
        (annualData.profitability.operatingProfit / annualData.revenue.total) * 100;
      annualData.profitability.netMargin =
        (annualData.profitability.netProfit / annualData.revenue.total) * 100;
      annualData.profitability.ebitda =
        annualData.profitability.operatingProfit + annualData.expenses.capitalExpenditures;
      annualData.profitability.ebitdaMargin =
        (annualData.profitability.ebitda / annualData.revenue.total) * 100;
  ...
  private generateComplianceRecommendations(issues: string[]): string[] {
    ...
    if (issues.includes('GAAP compliance issues detected')) {
      recommendations.push(
        'Review revenue recognition policies and ensure proper expense categorization'
      );
    }

    if (issues.includes('IFRS compliance issues detected')) {
      recommendations.push(
        'Ensure international financial reporting standards are properly implemented'
      );
    }
  ...
  async generateInvestorDeck(): Promise<void> {
    ...
    const deckData = {
      company: {
        ...
        mission:
          'To preserve and promote Tamil literature and natural products through technology and community',
