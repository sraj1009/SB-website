...
export const WishlistAPI = {
    ...
    addToWishlist: async (productId: string) => {
        const response = await apiRequest<{ success: boolean; message: string }>('/wishlist/' + productId, {
            method: 'POST'
        });
        return response;
    },
    ...
    checkWishlist: async (productId: string) => {
        const response = await apiRequest<{ success: boolean; data: { inWishlist: boolean } }>('/wishlist/check/' + productId);
...
export const ReviewsAPI = {
    getProductReviews: async (productId: string) => {
        const response = await apiRequest<{ success: boolean; data: { reviews: any[] } }>(`/reviews/${productId}`);
        return response.data.reviews;
    },

    addReview: async (productId: string, reviewData: { rating: number; comment: string }) => {
        const response = await apiRequest<{ success: boolean; message: string }>(`/reviews/${productId}`, {
            method: 'POST',
...
export const AccountAPI = {
    getAddresses: async () => {
        const response = await apiRequest<{ success: boolean; data: { addresses: any[] } }>('/addresses');
...
export const AdminAPI = {
    ...
    getUsers: async (params?: any) => {
        ...
        const response = await apiRequest<{ success: boolean; data: { users: any[]; pagination: any } }>(`/admin/users?${queryParams}`);
        return response.data;
    },

    getProducts: async (params?: any) => {
        const queryParams = new URLSearchParams(params).toString();
        const response = await apiRequest<{ success: boolean; data: { products: any[]; pagination: any } }>(`/admin/products?${queryParams}`);
        return response.data;
    },

    getOrders: async (params?: any) => {
        const queryParams = new URLSearchParams(params).toString();
        const response = await apiRequest<{ success: boolean; data: { orders: any[]; pagination: any } }>(`/admin/orders?${queryParams}`);
        return response.data;
    },

    updateOrderStatus: async (id: string, status: string) => {
        const response = await apiRequest<{ success: boolean; data: { order: any } }>(`/admin/orders/${id}/status`, {
            method: 'PATCH',
    ...
    markPaymentComplete: async (id: string, transactionId?: string) => {
        const response = await apiRequest<{ success: boolean; data: { order: any } }>(`/admin/orders/${id}/payment`, {
            method: 'PATCH',
...
export default {
    ...
    post: <T>(endpoint: string, body?: any, options?: RequestInit) => apiRequest<T>(endpoint, { ...options, method: 'POST', body: JSON.stringify(body) }),
    put: <T>(endpoint: string, body?: any, options?: RequestInit) => apiRequest<T>(endpoint, { ...options, method: 'PUT', body: JSON.stringify(body) }),
    del: <T>(endpoint: string, options?: RequestInit) => apiRequest<T>(endpoint, { ...options, method: 'DELETE' })
};
